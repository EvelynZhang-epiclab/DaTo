...
# CUDA_VISIBLE_DEVICES=5 python scripts/search_depth_v5.py
root_dir= '/dockerdata/yuuweizhang/projects/sd1/outputs/search_depth/v5'
...
class ConstrainedSampling(Sampling):
    ...
    def generate_individual(self):
        ...
        max1 = np.random.randint(3,self.max1)
        # 第一层：15个元素，最多5个12
        ...
        max2 = np.random.randint(4,self.max2)
        ...
        # 第三层：15个元素，最多5个12
        max3 = random.randint(3,self.max3)
...
class DiffusionOptimizationProblem(Problem): 
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        # 定义目标函数
        out["F"] = np.column_stack([-rewards*100, total_times])
        # 定义约束条件
        out["G"] = -rewards
...
algorithm = NSGA2(
    pop_size=10,
    sampling=ConstrainedSampling(steps=50, max1=6,max2=8,max3=6),
    crossover=SBX(prob=0.8, eta=15, vtype=float, repair=RoundingRepair()),
    mutation=PM(prob=0.8, eta=15, vtype=float, repair=RoundingRepair()),
