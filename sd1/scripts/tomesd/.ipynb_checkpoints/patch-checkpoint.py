...
def make_tome_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class ToMeBlock(block_class):
        ...
        def _forward(self, x: torch.Tensor, context: torch.Tensor = None) -> torch.Tensor:
            ...
            time8= torch.cuda.Event(enable_timing=True)
            time9= torch.cuda.Event(enable_timing=True)
            time0.record()
            m_a, m_c, m_m, u_a, u_c, u_m = compute_merge(x, self._tome_info)
            time1.record()
            # This is where the meat of the computation happens
            merge_x=m_a(self.norm1(x))
            time2.record()
            out1=self.attn1(merge_x, context=context if self.disable_self_attn else None)
            time3.record()
            x = u_a(out1) + x
            time4.record()
            out2=self.attn2(self.norm2(x), context=context,flag=True)
            x =  out2+ x
            ...
            tome_time['select_tome_method']+=time0.elapsed_time(time1)
            tome_time['merge_attn1']+=time1.elapsed_time(time2)
            tome_time['attn1']+=time2.elapsed_time(time3)
            tome_time['unmerge_attn1']+=time3.elapsed_time(time4)
            tome_time['cross']+=time4.elapsed_time(time5)
            tome_time['mlp']+=time5.elapsed_time(time6)
            tome_time['total']+=time0.elapsed_time(time6)
            
            print('===========Tome Forward Time=========')
            print('total: ',tome_time['total'])
            print('select_tome_method: ',tome_time['select_tome_method'])
            print('merge_attn1: ',tome_time['merge_attn1'])
            print('attn1: ',tome_time['attn1'])
            print('unmerge_attn1: ',tome_time['unmerge_attn1'])
            print('cross: ',tome_time['cross'])
            print('mlp: ',tome_time['mlp'])
            '''
            return x
