...
def main(prompt_file, image_folder, term):
    # 加载prompts
    prompts = load_prompts(prompt_file)
    
    # 初始化CLIPScore对象
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    clip_score_metric = CLIPScore(model_name_or_path="openai/clip-vit-large-patch14").to(device)

    images_num = count_files(image_folder)
    
    # 处理每一个prompt
    for idx in range(0, term):
        # print(idx, '/', term)
        prompt = prompts[idx]
        k = 0
        while True:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_folder", type=str, default="/root/autodl-tmp/stable-diffusion-main-1/outputs/sito/coco30k/samples")
