...
for x in x_values:
    ...
    # x=list(x)
    print('x',x)
    command = f"CUDA_VISIBLE_DEVICES=6 "\
                  f"python scripts/eval_strategy.py --n_iter 1 --n_samples 8 --W 512 --H 512 --ddim_steps 50 --plms --skip_grid " \
                  f"--from-file coco30k.txt --outdir outputs/eval_depth/search_old/coco30k_1 " \
                  f"--enable_deepcache --cache_strategy '{x}' " \
                  f"--enable_dato --prune_selfattn_flag --prune_strategy 0.4 --align_cfg --diff_gema 1 --noise_alpha 0.1"
    os.system(command)
